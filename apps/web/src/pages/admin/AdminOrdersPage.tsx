...
import { CheckCircle2, ChevronRight, ListChecks, Search, Filter, Package, User, CreditCard, Truck, ExternalLink, MoreVertical, MapPin, FileText, Box, Home, Check } from "lucide-react";
...
const AdminOrdersPage = () => {
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      {/* Desktop Table View */}
      <Card className="hidden lg:block bg-zinc-900/30 border-zinc-800 overflow-hidden">
          ...
          <Table>
            ...
            <TableBody>
              {paginatedOrders.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={8} className="h-32 text-center text-muted-foreground">Nenhum pedido encontrado.</TableCell>
                </TableRow>
              ) : (
                paginatedOrders.map((order) => (
                  <TableRow key={order.id} className="border-zinc-800 hover:bg-zinc-800/20 transition-colors">
                    <TableCell className="font-mono text-[10px] text-zinc-500">#{order.id.slice(-8).toUpperCase()}</TableCell>
                    <TableCell className="font-medium">{order.customerName}</TableCell>
                    <TableCell>
                      <Badge variant="outline" className="bg-zinc-900 border-zinc-700 font-normal">
                        {order.channel === "Loja Virtual" ? "🌐 Loja" : "🏷️ Externa"}
                      </Badge>
                    </TableCell>
                    <TableCell className="text-xs text-zinc-400 capitalize">{order.paymentGateway ?? "-"}</TableCell>
                    <TableCell className="text-xs text-zinc-400">{formatAddress(order.shippingProvider)}</TableCell>
      ...
      <div className="grid grid-cols-1 gap-4 lg:hidden">
        {paginatedOrders.map((order) => (
          <Card key={order.id} className="bg-zinc-900/40 border-zinc-800 active:scale-[0.98] transition-transform" onClick={() => openOrder(order.id)}>
            <CardContent className="p-4">
              <div className="flex justify-between items-start mb-3">
                <div className="space-y-1">
                  <p className="font-mono text-[10px] text-zinc-500">#{order.id.slice(-8).toUpperCase()}</p>
              ...
              <div className="grid grid-cols-2 gap-y-3 gap-x-2 text-sm mb-4">
                ...
                <div className="flex items-center gap-2 text-zinc-400">
                  ...
                  <span className="text-xs truncate">{formatAddress(order.shippingProvider)}</span>
                </div>
                <div className="flex items-center gap-2 text-zinc-400">
              ...
              <Button variant="outline" className="w-full bg-zinc-800 border-zinc-700 hover:bg-zinc-700 text-xs gap-2">
      ...
      {totalPages > 1 && (
        <div className="flex justify-center mt-6">
          <Pagination>
            <PaginationContent className="bg-zinc-900/50 border border-zinc-800 rounded-full px-2 py-1">
              <PaginationItem>
                <PaginationPrevious 
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent className="max-w-4xl w-[95vw] h-[90vh] md:h-auto overflow-y-auto bg-zinc-950 border-zinc-800 text-zinc-100">
          <DialogHeader>
          ...
          {selectedOrder ? (
            <Tabs defaultValue="logistics" className="w-full">
              <div className="flex overflow-x-auto pb-1 mb-4 no-scrollbar">
                <TabsList className="bg-zinc-900 border border-zinc-800 w-full justify-start md:justify-center h-12 p-1 gap-1">
              ...
              <TabsContent value="logistics" className="space-y-6 focus-visible:outline-none">
                {/* Horizontal Stepper - Premium Style */}
                <div className="bg-zinc-900/50 border border-zinc-800 rounded-3xl p-6 md:p-8 overflow-x-auto no-scrollbar">
                  <div className="min-w-[700px] relative flex justify-between items-center px-4">
                    {/* Connecting Lines Container - Background line */}
                    <div className="absolute top-[24px] left-0 w-full h-[2px] bg-zinc-800 -translate-y-1/2 z-0 px-10" />
                    
                    {selectedOrder.steps
                      .sort((a, b) => {
                        const order = ["created", "payment_confirmed", "in_separation", "ready_to_ship", "shipped", "out_for_delivery", "delivered"];
                      ...
                      .map((step, idx, arr) => {
                        ...
                        return (
                          <div key={step.key} className="relative z-10 flex flex-col items-center gap-3">
                            {/* Line connecting to next step - Positioned behind circles */}
                            {showLine && (
                              <div className={cn(
                            ...
                            {/* Circle - Positioned in front of lines */}
                            <div 
                              className={cn(
                                "w-12 h-12 rounded-full border-2 flex items-center justify-center transition-all duration-500 relative z-20",
                                isCompleted 
                                  ? "bg-gradient-to-br from-primary to-orange-500 border-none shadow-[0_0_20px_rgba(var(--primary-rgb),0.4)]" 
                                  : isActive
                                  ? "bg-zinc-950 border-primary ring-4 ring-primary/20 scale-110"
                                  : isNext
                                  ? "bg-zinc-950 border-blue-500 animate-pulse"
                                  : "bg-zinc-950 border-zinc-800 opacity-100"
                              )}
                            ...
                            >
                              {isCompleted ? (
                                ...
                                <StepIcon className={cn("w-5 h-5", isActive ? "text-primary" : "text-zinc-500")} />
                              )}
                            </div>
                            ...
                            <div className="text-center h-12 flex flex-col items-center justify-start">
                              <p className={cn(
                                ...
                                isActive ? "text-primary" : isCompleted ? "text-zinc-300" : "text-zinc-500"
                              )}>
                                {step.label}
                              </p>
                              {isActive && (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {/* Address Section - Moved to first tab */}
                    ...
                    {selectedOrder.shippingAddress ? (
                      <div className="space-y-6">
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            <p className="text-[10px] text-zinc-500 uppercase font-bold tracking-widest mb-1">Rua e Número</p>
                            <p className="text-sm font-medium">{selectedOrder.shippingAddress.street}, {selectedOrder.shippingAddress.number}</p>
                          </div>
                          <div>
                            <p className="text-[10px] text-zinc-500 uppercase font-bold tracking-widest mb-1">Bairro</p>
                            <p className="text-sm font-medium">{selectedOrder.shippingAddress.neighborhood}</p>
                          </div>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            <p className="text-[10px] text-zinc-500 uppercase font-bold tracking-widest mb-1">Cidade/Estado</p>
                            <p className="text-sm font-medium">{selectedOrder.shippingAddress.city} - {selectedOrder.shippingAddress.state}</p>
                          </div>
                          <div>
                            <p className="text-[10px] text-zinc-500 uppercase font-bold tracking-widest mb-1">CEP</p>
                            <p className="text-sm font-medium">{selectedOrder.shippingAddress.zipCode}</p>
                          </div>
                        </div>
                        <Button 
                          variant="outline" 
                          size="sm" 
                          className="w-full bg-zinc-800 border-zinc-700 hover:bg-zinc-700 gap-2 h-10 font-bold rounded-xl"
                          onClick={() => {
                            const addr = `${selectedOrder.shippingAddress?.street}, ${selectedOrder.shippingAddress?.number}${selectedOrder.shippingAddress?.complement ? ` - ${selectedOrder.shippingAddress?.complement}` : ''}, ${selectedOrder.shippingAddress?.neighborhood}, ${selectedOrder.shippingAddress?.city} - ${selectedOrder.shippingAddress?.state}, CEP: ${selectedOrder.shippingAddress?.zipCode}`;
                      ...
                      <div className="py-8 text-center border border-dashed border-zinc-800 rounded-2xl">
                        <p className="text-sm text-zinc-500">Dados de endereço não disponíveis.</p>
                      </div>
                    )}
                  </div>

                  {/* Actions Section */}
                  <div className="flex flex-col justify-center gap-4 bg-zinc-900/30 border border-zinc-800 rounded-3xl p-6">
                    <div className="text-center space-y-2 mb-4">
                       <p className="text-[10px] text-zinc-500 uppercase font-bold tracking-widest">Status Atual</p>
                       <h4 className="text-2xl font-black text-primary uppercase">{selectedOrder.statusLabel}</h4>
                    </div>
                    
                    <Button 
                      className="w-full gap-2 bg-gradient-to-r from-primary to-orange-500 text-black font-black hover:opacity-90 h-16 rounded-2xl shadow-[0_10px_30px_rgba(var(--primary-rgb),0.2)] transition-all active:scale-95 text-lg" 
                      onClick={nextStep}
                    >
                      {selectedOrder.currentStep === "delivered" ? "Pedido Concluído" : "Avançar para Próxima Etapa"} <ChevronRight className="w-5 h-5" />
                    </Button>
                    <p className="text-[10px] text-center text-zinc-500">Clique no ícone de qualquer etapa para definir manualmente o status.</p>
                  </div>
                </div>
              </TabsContent>
              ...
              <TabsContent value="customer" className="space-y-4 focus-visible:outline-none">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-4 flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-[10px] text-zinc-500 uppercase font-bold tracking-widest mb-1">Cliente</p>
                      <p className="font-bold text-base">{selectedOrder.customerName}</p>
                    </div>
                  </div>
                  <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-4 flex items-center gap-4 text-emerald-500">
                    <div className="p-3 bg-emerald-500/10 rounded-xl"><CreditCard className="w-6 h-6" /></div>
                    <div>
                      <p className="text-[10px] text-zinc-500 uppercase font-bold tracking-widest mb-1">Pagamento</p>
                ...
                <div className="bg-zinc-900/30 border border-zinc-800 rounded-2xl p-6">
                   <p className="text-sm text-zinc-400">Outras informações do cliente podem ser visualizadas no menu <span className="text-primary font-bold">Clientes</span>.</p>
                </div>
              </TabsContent>
                ...
                <div className="bg-zinc-900/30 border border-zinc-800 rounded-2xl p-4 md:p-6">
                  ...
                  <div className="space-y-3">
                    {selectedOrder.items.map((item, index) => (
                      <div key={`${item.productName}-${index}`} className="flex items-center justify-between p-4 rounded-xl bg-zinc-900/50 border border-zinc-800/50 hover:border-zinc-700 transition-colors">
                        <div className="flex gap-4 items-center">
                          <div className="w-16 h-16 rounded-lg bg-zinc-800 flex items-center justify-center overflow-hidden border border-zinc-700">
                            {item.productImage ? (
                          ...
                          <div className="flex flex-col">
                            <span className="text-sm font-bold text-zinc-100">{item.productName}</span>
                            <span className="text-[11px] text-zinc-500">Qtd: <span className="text-zinc-300 font-bold">{item.quantity}</span></span>
                          </div>
                        </div>
                        <div className="text-right">
                          ...
                          <p className="text-[10px] text-zinc-600">unitário</p>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <div className="h-64 flex items-center justify-center">
              <div className="flex flex-col items-center gap-3">
                ...
                <p className="text-zinc-500 animate-pulse">Carregando detalhes...</p>
              </div>
            </div>
          )}

          <DialogFooter className="mt-4 pt-4 border-t border-zinc-800 flex flex-row items-center justify-end gap-2">
            <Button variant="ghost" className="hover:bg-zinc-900 text-zinc-400" onClick={() => setModalOpen(false)}>Fechar</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
