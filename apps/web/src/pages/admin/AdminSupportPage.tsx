...
export default function AdminSupportPage() {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        ...
        if (selectedTicket) {
          ...
          if (updated) {
            // Só atualiza se houver mudança real ou se for pra preencher dados
            setSelectedTicket(prev => {
              if (!prev) return updated;
              // Mantém as mensagens atuais se as novas vierem vazias (segurança)
  ...
  const handleStatusUpdate = async (id: string, status: TicketStatus) => {
    try {
      console.log(`[AdminSupport] Atualizando status do ticket ${id} para ${status}...`);
      const updated = await adminRepository.updateTicketStatus(id, status);
      console.log(`[AdminSupport] Sucesso! Novo status:`, updated.status);
      ...
      console.error("[AdminSupport] Erro ao atualizar status:", error);
      ...
      <div className="flex-1 grid grid-cols-12 gap-6 overflow-hidden">
        ...
        <div className="col-span-12 lg:col-span-4 flex flex-col space-y-4 overflow-hidden">
          <Card className="flex-1 flex flex-col bg-card border-border overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-2 scrollbar-premium">
                ...
                paginatedTickets.map(ticket => (
                  ...
                  >
                    ...
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <Badge className={`text-[9px] h-5 font-bold uppercase tracking-tighter ${
                  ...
                  <PaginationContent className="gap-1">
                    <PaginationItem key="prev">
                      <PaginationPrevious 
                        ...
                        className={page === 1 ? "pointer-events-none opacity-20 h-8 w-8 p-0" : "cursor-pointer h-8 w-8 p-0"}
                      />
                    </PaginationItem>
                    <span key="info" className="text-[10px] font-bold px-2">{page}/{totalPages}</span>
                    <PaginationItem key="next">
                      <PaginationNext 
                        onClick={() => setPage(p => Math.min(totalPages, p + 1))}
                        className={page === totalPages ? "pointer-events-none opacity-20 h-8 w-8 p-0" : "cursor-pointer h-8 w-8 p-0"}
                      />
                    </PaginationItem>
                  </PaginationContent>
                </Pagination>
              ...
              <>
                ...
                <div className="p-4 border-b border-border bg-secondary/20 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  ...
                  <div className="flex items-center gap-3 w-full sm:w-auto">
                    ...
                    >
                      <SelectTrigger className={`h-9 w-[160px] font-bold text-xs ${
                ...
                {/* Área de Mensagens */}
                <div className="flex-1 overflow-y-auto p-4 space-y-6 bg-[#0a0a0a]/30 scrollbar-premium">
                  {/* Descrição Inicial */}
                  ...
                  {selectedTicket?.messages?.map((msg) => (
                    <div key={msg.id || Math.random().toString()} className={`flex flex-col space-y-1 ${msg.senderRole === "CUSTOMER" ? "items-start" : "items-end"}`}>
                      ...
                      <span className={`text-[10px] text-muted-foreground ${msg.senderRole === "CUSTOMER" ? "ml-2" : "mr-2"}`}>
                  ...
                  <div className="p-4 bg-card border-t border-border space-y-4">
                    ...
                    <div className="flex gap-3 items-end">
                      ...
                      <Button 
                        ...
                        className="h-12 px-6 font-bold shimmer-btn"
                      >
                        {isUploading ? (
